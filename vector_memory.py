...
from .hybrid_router import validate_azure_endpoint
...
class OllamaEmbeddingFunction:
    ...
    def _embed_via_azure(self, texts: list[str]) -> list[list[float]] | None:
        ...
        base = validate_azure_endpoint(AZURE_OPENAI_ENDPOINT).rstrip("/")
        if base.endswith("/openai/v1"):
            ...
            payload = {
                ...
                "input": texts,
                "dimensions": FALLBACK_EMBED_DIM,
            }
        else:
            url = (
                f"{base}/openai/deployments/{AZURE_EMBED_DEPLOYMENT}/embeddings"
                f"?api-version={AZURE_OPENAI_API_VERSION}"
            )
            headers = {"api-key": AZURE_OPENAI_API_KEY}
            payload = {"input": texts, "dimensions": FALLBACK_EMBED_DIM}
        with httpx.Client(timeout=EMBED_TIMEOUT) as client:
