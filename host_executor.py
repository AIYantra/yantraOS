...
def _execute_locked_intent(intent: str, target: str, started: float) -> dict[str, Any]:
    audit_action = {"action": intent, "target": target}
    ...
    def audited(response: dict[str, Any]) -> dict[str, Any]:
        phase = "EXECUTED" if response["status"] == "SUCCESS" else "FAILED"
        if audit_log.log_action(phase=phase, action=audit_action):
            return response
        return _response(
            "FATAL", intent=intent, target=target, error="Privileged audit unavailable."
        )
    ...
    if _is_live_iso():
        if intent in _DESTRUCTIVE_INTENTS:
            ...
            return audited(
                _response("BLOCKED", intent=intent, target=target, error=message)
            )
        if intent not in _SAFE_LIVE_INTENTS:
            return audited(
                _response("REJECTED", intent=intent, error="Intent is not allowed on Live ISO.")
            )
    else:
