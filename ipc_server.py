...
CONTROL_PORT = 50000
CONTROL_SOCKET_PATH = "/run/yantra-control/control.sock"
...
def _is_control_socket_scope(scope: dict) -> bool:
    server = scope.get("server")
    return (
        scope.get("client") is None
        and isinstance(server, (list, tuple))
        and len(server) == 2
        and server[0] == CONTROL_SOCKET_PATH
...
class InjectCommand(BaseModel):
    ...
    command: StrictStr = Field(min_length=1, max_length=MAX_COMMAND_CHARS)
...
def attach_ipc_routes(app: FastAPI, engine_ref) -> None:
    ...
    async def guard_control_plane(request: Request, call_next):
        ...
        unix_client = _is_control_socket_scope(request.scope)
        if not unix_client:
