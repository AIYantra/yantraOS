    ...
    if isinstance(value, dict):
        ...
        for key, item in items[:_MAX_AUDIT_ITEMS]:
            ...
            if _is_secret_field(key) or (
                isinstance(key, str) and key.lower() in _SENSITIVE_ACTION_FIELDS
            ):
...
def _append_payload(payload: dict[str, Any]) -> None:
    ...
    try:
        ...
        # Reserve the final hash before truncation so the canonical data is the
        # same data written to disk.
        chained["entry_hash"] = "0" * 64
        _encode_payload(chained)
        canonical_payload = dict(chained)
        del canonical_payload["entry_hash"]
...
def log_execution(script: str, result: "SandboxResult") -> bool:
    ...
    try:
        ...
        log.error("> AUDIT: Failed to append to audit log: %s", type(exc).__name__)
        return False
...
def log_action(
    *,
    ...
    try:
        ...
        action_detail = _sanitize_action(action)
        payload: dict[str, Any] = {
            ...
            "action_id": hashlib.sha256(
                json.dumps(action_detail, ensure_ascii=False, sort_keys=True).encode("utf-8")
            ).hexdigest(),
        }
        ...
        if result is not None:
            payload["result"] = _metadata(result, "redacted")
        if error is not None:
            payload["error"] = _metadata(error, "redacted")

        _append_payload(payload)
        return True
    except Exception as exc:
        log.error("> AUDIT: Failed to append action log: %s", type(exc).__name__)
        return False
