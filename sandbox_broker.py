...
RESPONSE_TIMEOUT_SECS: Final[int] = 5
CLOSE_TIMEOUT_SECS: Final[int] = 1
MAX_ACTIVE_REQUESTS: Final[int] = 2
...
async def _write_result(writer: Any, result: SandboxResult) -> None:
    ...
    await asyncio.wait_for(writer.drain(), timeout=RESPONSE_TIMEOUT_SECS)


async def _close_writer(writer: Any) -> None:
    writer.close()
    try:
        await asyncio.wait_for(writer.wait_closed(), timeout=CLOSE_TIMEOUT_SECS)
...
class SandboxBroker:
    ...
    async def _handle_client(self, reader: Any, writer: Any) -> None:
        ...
        try:
            ...
            if not log_action(phase="PROPOSED", action=audit_action):
                await _write_result(
                    writer,
                    _error_result(
                        ExecOutcome.DOCKER_ERROR,
                        "root sandbox proposal audit failed",
                    ),
                )
                return
            ...
            terminal_phase = (
                "EXECUTED" if result.outcome == ExecOutcome.SUCCESS else "FAILED"
            )
            audit_ok = log_action(
                phase=terminal_phase,
                action=audit_action,
                result=(
                    f"outcome={result.outcome.value}; exit={result.exit_code}"
                ...
                error=(
                    None
                    if result.outcome == ExecOutcome.SUCCESS
                    else result.error or result.stderr or result.outcome.value
                ),
            )
            if not audit_ok:
                result = _error_result(
                    ExecOutcome.DOCKER_ERROR,
                    "root sandbox outcome audit failed",
                )
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            if (
                ...
                raise RuntimeError("unsafe stale broker socket path")
            with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as probe:
                result = probe.connect_ex(BROKER_SOCKET_PATH)
            if result == 0:
                raise RuntimeError("sandbox broker socket is already active")
            if result not in {errno.ECONNREFUSED, errno.ENOENT}:
                raise OSError(result, os.strerror(result), BROKER_SOCKET_PATH)
    ...
    async def close(self) -> None:
        ...
        if self._owns_socket:
            try:
                existing = os.lstat(BROKER_SOCKET_PATH)
                if (
                    stat.S_ISSOCK(existing.st_mode)
