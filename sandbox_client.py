...
BROKER_RESPONSE_TIMEOUT_SECS: Final[int] = 40
BROKER_WRITE_TIMEOUT_SECS: Final[int] = 5
BROKER_CLOSE_TIMEOUT_SECS: Final[int] = 1
...
async def _close_writer(writer: Any) -> None:
    ...
    try:
        await asyncio.wait_for(
            writer.wait_closed(), timeout=BROKER_CLOSE_TIMEOUT_SECS
        )
...
class SandboxClient:
    ...
    async def _open_verified(self) -> tuple[Any, Any]:
        socket_info = _stat(self._socket_path, follow_symlinks=False)
    ...
    async def execute(self, script: str) -> SandboxResult:
        ...
        try:
            ...
            await asyncio.wait_for(
                writer.drain(), timeout=BROKER_WRITE_TIMEOUT_SECS
            )
