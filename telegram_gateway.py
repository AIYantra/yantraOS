...
DEBUG_URL = "http://127.0.0.1:50000/debug"
    ...
    async with _engine_session() as session:
        try:
            async with session.get(
                ...
                lines = [
                    "=== YantraOS Debug Diagnostics ===",
                    f"Router local_only: {_bounded_text(data.get('router_local_only', 'UNKNOWN'), 128)}",
                    f"Router last_tier: {_bounded_text(data.get('router_last_tier', 'UNKNOWN'), 128)}",
                    "",
                    _bounded_text(data.get("journal_tail", data.get("journal", "No journal output.")), 2000),
                ]
                report = "\n".join(lines)
                await safe_send(None, report, message, is_reply=True)
        except Exception:
