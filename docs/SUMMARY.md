# Table of contents

* [Introduction](README.md)

## Core Architecture

* [The Kriya Loop (Daemon)](architecture/kriya-loop.md)
* [Ephemeral Docker Sandbox](architecture/docker-sandbox.md)
* [IPC Bridge & UNIX Sockets](architecture/ipc-bridge.md)

## The Skill Store

* [Vector Memory Topology (Pinecone)](the-skill-store/vector-memory-topology-pinecone.md)
* [yantraos/skill/v1 Schema](skills/schema.md)

## Security & Isolation

* [The Amnesia Protocol](security/amnesia-protocol.md)
* [Hardware Routing & Fallbacks](security/hardware-routing.md)
* [IPC Bridge Hardening](security/ipc-hardening.md)
